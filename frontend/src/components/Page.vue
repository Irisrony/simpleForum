  ...
  <div class="body-box">
      ...
      <el-form :model="loginForm" id="header_form">
        ...
        <div>
          ...
          <el-row>
            ...
            <el-link @click="registerFormVisible = true" v-show="!isLogin"
              >立即注册</el-link
            >

            <!--  登录后功能区  -->
            <el-button @click="postsFormVisible = true" v-show="isLogin"
              >发帖</el-button
            >
            <el-button v-show="isLogin" disabled>删除</el-button>
    ...
    <footer>
      <div class="foot_stat">
        <p>
          <b
            >免责声明：本站所有帖子内容以及链接均为虚构，如有侵权问题，请及时联系，我们将尽快处理.</b
          >
        </p>
      </div>
    </footer>
    ...
    <el-dialog v-model="registerFormVisible" title="用户注册">
      ...
      >
        ...
        <el-form-item label="确认密码" prop="okpassword">
          <el-input
            type="password"
            v-model="registerForm.okpassword"
          ></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="uploadRegister"> 确认 </el-button>
        </span>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="postsFormVisible" title="发帖区">
      ...
      >
        ...
        <QuillEditor
          ...
          plcaeholder="请输入帖子内容"
          theme="snow"
          v-model:content="postsForm.content"
          style="height: 200px"
        ></QuillEditor>
      </el-form>
      <template #footer>
        <p>
          您还可以输入
          <strong>{{ 100 - postsForm.content.length }}</strong> 个字
        </p>
        <span class="dialog-footer">
          <el-button @click="postsFormVisible = false">取消</el-button>
          <el-button type="primary" @click="uploadPosts"> 确认 </el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
export default {
  ...
  created() {
    this.queryForm.id = sessionStorage.getItem('id')
  ...
  methods: {
    // 请求分页查询数据
    getPost() {
      request.get('/posts/onePage', { params: this.queryForm }).then((res) => {
